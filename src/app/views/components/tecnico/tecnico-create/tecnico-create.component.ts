...
export class TecnicoCreateComponent implements OnInit {
  ...
  public create(): void {
    this.tecnicoService.create(this.tecnico).subscribe((resposta) => {
      this.router.navigate(['tecnicos']),
      this.tecnicoService.generateMessage('Técnico cadastrado com sucesso!')
    }, err => {
      if (err.error.error.match('já cadastrado')) {
        this.tecnicoService.generateMessage(err.error.error);
      }
      else if (err.error.errors[0].message === "número do registro de contribuinte individual " 
      + "brasileiro (CPF) inválido"){
        this.tecnicoService.generateMessage("CPF inválido");
      }
      else {
        console.log(err);
      }
